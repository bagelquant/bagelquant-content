...
alternate_lang_url: ../../../../../cn/docs/core/reference/transformers/rolling_max.md
nav: docs_en
---
...
```python
rolling_max(source, window, min_periods=None, name=None, metadata=None)
```
...
from bagelquant_core.transformer import rolling_max

domain = Domain(calendar=pd.to_datetime(["2024-01-02", "2024-01-03", "2024-01-04"]), universe=["a", "b"])
source = Panel.from_domain(pd.DataFrame({"a": [1.0, 2.0, 4.0], "b": [2.0, 3.0, 8.0]}, index=domain.sessions), domain)

result = rolling_max(source, window=2).compute().data
print(result)
```
